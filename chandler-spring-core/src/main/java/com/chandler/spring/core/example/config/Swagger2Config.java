...
public class Swagger2Config {
	...
	public Docket createRestApi() {
		return new Docket(DocumentationType.SWAGGER_2).pathMapping("/").select()
				.apis(RequestHandlerSelectors.basePackage("com.chandler.spring.core.example.controller"))// 指定方法接口都来自controller这个包
				.paths(PathSelectors.any())// 选any表示给这个controller包下所有的接口都生成文档
				.build()
				.apiInfo(new ApiInfoBuilder().title("服务提供者工程swagger页面")// 生成的接口文档的标题名称
